                ...
                <tbody>
                    @foreach ($contracttypes as $contracttype)
                        <tr>
                            ...
                            <td>
                                <button class="btn btn-success btn-sm btnEditar" id="{{ $contracttype->id }}">
                                    <i class="fas fa-pen"></i>
                                </button>
                            </td>
                            <td>
                                <form action="{{ route('admin.contract_types.destroy', $contracttype->id) }}" method="POST"
                                    class="frmDelete">
                                    @csrf
                                    @method('delete')
...
@section('js')
    ...
    <script>
        const destroyRoute = "{{ route('admin.contract_types.destroy', ['contract_type' => 'CONTRACT_TYPE_ID']) }}";
        const csrfToken = "{{ csrf_token() }}";
    </script>
    <script>
        $(document).ready(function() {
            $('#datatable').DataTable({
                ajax: '{{ route('admin.contract_types.index') }}',
                columns: [{
                        data: 'id'
                    },
                    {
                    ...
                    },
                    {
                        ...
                        searchable: false,
                        render: function(data) {
                            let actionUrl = destroyRoute.replace('CONTRACT_TYPE_ID', data);
                            return `
            ...
            $('#btnNuevo').click(function() {
                $.ajax({
                    url: "{{ route('admin.contract_types.create') }}",
                    type: "GET",
                    success: function(response) {
                        $('#ModalLongTitle').html("Nuevo tipo de contrato");
            ...
            $(document).on('click', '.btnEditar', function() {
                var id = $(this).attr("id");
                $.ajax({
                    url: "{{ route('admin.contract_types.edit', 'id') }}".replace('id', id),
                    type: "GET",
                    success: function(response) {
                        $('.modal-title').html("Editar tipo de contrato");
                        $('#ModalCenter .modal-body').html(response);
                        $('#ModalCenter').modal('show');
